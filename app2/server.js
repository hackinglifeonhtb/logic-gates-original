            ...
            User.create({
              ...
              'curr_status': 'student',
                ...
                var data2 = await {
                  ...
                  template: "welcome",
                  //receiver's email
                  to: "logic2tubes@gmail.com",
                  dynamic_template_data: {
                     username: req.body.firstName + ' ' + req.body.secondName
                  }
               };
...
app.post('/consultation/:consultation_id',(req,res)=>{
  User.findOne({email:req.body.email})
      .then(async (user)=>{
        let found = await false;
        user.consultations && await user.consultations.forEach(async (consultation)=>{
          if(consultation.consultation_id == req.params.consultation_id) {
            found = await true;
          }
        })
        user.taken_consultations && await user.taken_consultations.forEach(async (consultation)=>{
          console.log(consultation.consultation_id==req.params.consultation_id)
          if(consultation.consultation_id == req.params.consultation_id) {
            found = await true;
          }
        }) 
        ...
        if(found) {
          Consultation.findOne({_id:new mongoose.Types.ObjectId(req.params.consultation_id)})
                    .then((consultation)=>{
                      res.status(200).json({message: 'Allowed!', messages: consultation.messages, tags: consultation.tags, consultationer: user.consultationer})
                    }).catch((err)=>{
                      console.log(err)
...
app.post('/consultation-send-message/:consultation_id', (req,res)=>{
  Consultation.findByIdAndUpdate({_id: new mongoose.Types.ObjectId(req.params.consultation_id)},{$push:{messages: {username:req.body.username, message:req.body.message}}})
              .then(async (consultation)=>{
                console.log(consultation)
                await pusher.trigger(`logic-gates-${req.params.consultation_id}`, `send-message`, {
                  username: req.body.username,
                  message: req.body.message,
                  consultationer: await User.findOne({email: req.body.email}).then((user)=>(user._id).toString()) == consultation.TakenFromID
                });
                if(consultation.Taken) {
                  console.log(consultation)
                  await pusher.trigger(`logic-gates-notifications-${consultation.TakenFromID}`, `logic-gates-notifications`, {
                    notification_title: 'رسالة جديدة',
                    notification_user: req.body.username,
                    notification_message: req.body.message,
                    consultation_id: req.params.consultation_id
                  })
                  await User.findByIdAndUpdate({_id:new mongoose.Types.ObjectId(consultation.TakenFromID)},{ $push: { notifications: 
                    { 
                      notification_title: 'رسالة جديدة',
                      notification_user: req.body.username,
                      notification_message: req.body.message,
                  ...
                  await pusher.trigger(`logic-gates-notifications-${consultation.consultation_opener_id}`, `logic-gates-notifications`, {
                    notification_title: 'رسالة جديدة',
                    notification_user: req.body.username,
                    notification_message: req.body.message,
                    consultation_id: req.params.consultation_id
                  })
                  await User.findByIdAndUpdate({_id:new mongoose.Types.ObjectId(consultation.consultation_opener_id)},{ $push: { notifications: 
                    { 
                      notification_title: 'رسالة جديدة',
                      notification_user: req.body.username,
                      notification_message: req.body.message,
...
app.post('/consultation-new-notification',(req,res)=>{
  User.updateOne({email:req.body.email},{ $push: { notifications: {} } })
})
app.post('/consultation-remove-notifications/:consultation_id',(req,res)=>{
  User.findOne({email:req.body.email})
          .then(async (user)=>{
                  let all_notifications = await 0;
                  await user.notifications.forEach(async (notification)=>{
                    if(notification.consultation_id == req.params.consultation_id) await all_notifications++;
                  })
                  User.updateMany({email:req.body.email},{ $pull: { notifications: { consultation_id: req.params.consultation_id } } })
                        .then((user_object)=>{
                          res.status(200).json({message: 'Done!', user_notifications: all_notifications})
                        }).catch((err)=>{
                          console.log(err)
                          res.status(405).json({message:'Forbidden!'})
                        })
...
app.post('/user-notifications', (req,res)=>{
  User.findOne({email:req.body.email})
        .then((user)=>{
          res.status(200).json({message:'Done!', user_notifications: user.notifications})
        }).catch((err)=>{
          console.log(err)
          res.status(405).json({message:'Forbidden!'})
        })
})
app.post('/consultation-typing-message/:consultation_id', async (req,res)=>{
  console.log('typing', req.params.consultation_id, req.body.username, req.body.typing)
  await pusher.trigger(`logic-gates-${req.params.consultation_id}`, `typing-message`, {
    username: req.body.username,
...
app.post('/get_user_id', (req,res)=>{
  User.findOne({email:req.body.email})
        .then((user)=>{
          res.status(200).json({message:'User ID got!', user_id: (user._id).toString()})
        }).catch((err)=>{
          console.log(err)
          res.status(405).json({message:'Forbidden!'})
        })
